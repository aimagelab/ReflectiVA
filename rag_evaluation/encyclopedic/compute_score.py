...
def main():
  ...
  for p in args.input_path:
    ...
    two_hop_scores = []
    all_scores = []
    for element in tqdm(data, mininterval=1):
      score = evaluate_example(question=element['question'], reference_list=element['reference'], candidate=element['answers'], question_type=element['question_type'])
    ...
    single_score = sum(single_hop_scores) / len(single_hop_scores)
    all_score = sum(all_scores) / len(all_scores)
    two_score = sum(two_hop_scores) / len(two_hop_scores)
    ...
    with open(f'{p}/results.json', 'w') as f:
